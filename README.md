...
- macOS 26 及以上可通过控制中心的「打开 App」卡片呼出 Luma。
...
在 macOS 26 及以上，打开「控制中心 → 编辑控制」，找到「快捷指令 → 打开 App」，添加后将目标 App 设为 Luma。点击这张卡片会打开 Luma 面板；Luma 未运行时也可以启动。

卡片使用系统自带的「打开 App」控制，不需要安装控件扩展或新增隐私权限。控制中心布局由用户配置，不会随应用安装自动添加。预设文本、剪贴板、JSON 和 Diff 仍在 Luma 面板中操作；`⌥ Space` 与菜单栏入口也可继续使用。
